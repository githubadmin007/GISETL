...
// 获取缓存的菜单
function _GetContextMenuCache(menuId) {
    if (menuId) {
...
// 创建菜单
function _CreateContextMenu(options, vnode) {
    let NestListCfg = {
...
function GetContextMenu(options, vnode) {
    let ContextMenu = _GetContextMenuCache(options.menuId);
    if (!ContextMenu) {
        ...
        ContextMenu.$data.NestListCfg = options;
    }
    return ContextMenu;
}
